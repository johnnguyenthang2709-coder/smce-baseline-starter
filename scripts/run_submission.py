...
def main() -> None:
    ...
    image_paths = {p.stem: p for p in img_dir.glob("*.jpg")}
    image_paths.update({p.stem: p for p in img_dir.glob("*.jpeg")})
    image_paths.update({p.stem: p for p in img_dir.glob("*.png")})
    ordered_ids = [str(iid) for iid in catalog["image_id"].tolist()]
    available = [iid for iid in ordered_ids if iid in image_paths]
    if args.limit:
    ...
    done = {r["image_id"]: r for r in rows}
    final_rows = []
    for iid in catalog["image_id"]:
        iid = str(iid)
        if iid in done:
            final_rows.append(done[iid])
        else:
            final_rows.append(
                {
                    "image_id": iid,
    ...
    out.to_csv(out_path, index=False, encoding="utf-8-sig", quoting=csv.QUOTE_ALL)
